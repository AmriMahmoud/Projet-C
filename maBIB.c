...
RESULTAT* trouverTop3ClientsParAnnee(FILE *fr, int annee) {
    rewind(fr);
    ...
    while (fread(&ren, sizeof(RENDEZVOUS), 1, fr)) {
        ...
        int trouve = 0;
        RESULTAT* actuel = clients;
        for (int i = 0; i < taille; i++) {
            if (actuel->id == ren.client.CIN) {
                actuel->nombretraitement++;
                trouve = 1;
                break;
            }
        }
    ...
    for (RESULTAT* i = clients; i < clients + taille - 1; i++) {
        for (RESULTAT* j = i + 1; j < clients + taille; j++) {
            if (i->nombretraitement < j->nombretraitement) {
                RESULTAT temp = *i;
                *i = *j;
                *j = temp;
            }
    ...
    RESULTAT* top3 = malloc(3 * sizeof(RESULTAT));
    RESULTAT* c = clients;
    RESULTAT* t = top3;
    for (int i = 0; i < 3 && i < taille; i++, c++, t++) {
        *t = *c;
    }
    free(clients);
    return top3;
}
RESULTAT** creerTableauTop3ParAnnee(FILE *fr, int annee_min, int annee_max) {
    int nombre_annee = annee_max - annee_min + 1;
    RESULTAT** tableau = malloc(nombre_annee * sizeof(RESULTAT*));

    for (RESULTAT** t = tableau; t < tableau + nombre_annee; t++) {
        int annee_actuel = annee_min + (t - tableau);
        *t = trouverTop3ClientsParAnnee(fr, annee_actuel);
    }
    return tableau;
}
void afficherTableauTop3ParAnnee(RESULTAT** tableau, int annee_min, int annee_max) {
    int nombre_annee = annee_max - annee_min + 1;
    RESULTAT** t = tableau;
    for (int i = 0; i < nombre_annee; i++, t++) {
        printf("Année %d :\n", annee_min + i);
        RESULTAT* top3 = *t;
        for (RESULTAT* client = top3; client < top3 + 3; client++) {
...
void libererTableauTop3ParAnnee(RESULTAT** tableau, int annee_min, int annee_max) {
    int nombre_annee = annee_max - annee_min + 1;
    for (RESULTAT** t = tableau; t < tableau + nombre_annee; t++) {
        free(*t);
    }
